...
export default async function ClientesPage() {
  ...
  async function createClientAction(formData: FormData) {
    ...
    await createClient(currentUser, {
  ...
  async function removeClientAction(formData: FormData) {
    'use server';
    const currentUser = await requireUserContext();
    await deleteClient(currentUser, { clientId: String(formData.get('clientId') || '') });
    revalidatePath('/clientes');
    revalidatePath('/tramites');
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-white">Base de clientes en PostgreSQL</h1>
          <p className="mt-3 text-sm leading-6 text-slate-300">Alta y consulta reales, compartidas entre dispositivos y usuarios del negocio.</p>
        ...
        <form action={createClientAction} className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="block"><span className="mb-2 block text-sm text-slate-300">Email</span><input name="email" type="email" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" /></label>
