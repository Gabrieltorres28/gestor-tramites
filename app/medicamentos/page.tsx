...
export default async function MedicamentosPage() {
  ...
  async function addMedicineAction(formData: FormData) {
    ...
    await createMedicine(currentUser, { name: String(formData.get('name') || ''), supplier: String(formData.get('supplier') || ''), purchasePrice: Number(formData.get('purchasePrice') || 0), salePrice: Number(formData.get('salePrice') || 0) });
  ...
  async function addBatchAction(formData: FormData) {
    'use server';
    const currentUser = await requireUserContext();
    await createMedicineBatch(currentUser, { medicineId: String(formData.get('medicineId') || ''), batchNumber: String(formData.get('batchNumber') || ''), expirationDate: String(formData.get('expirationDate') || ''), quantityAvailable: Number(formData.get('quantityAvailable') || 0) });
    revalidatePath('/medicamentos');
    revalidatePath('/');
  }
  ...
  async function removeMedicineAction(formData: FormData) {
    'use server';
    const currentUser = await requireUserContext();
    await deleteMedicine(currentUser, { medicineId: String(formData.get('medicineId') || '') });
    revalidatePath('/medicamentos');
    revalidatePath('/');
  }
  ...
  async function removeBatchAction(formData: FormData) {
    'use server';
    const currentUser = await requireUserContext();
    await deleteMedicineBatch(currentUser, { batchId: String(formData.get('batchId') || '') });
    revalidatePath('/medicamentos');
    revalidatePath('/');
  }
