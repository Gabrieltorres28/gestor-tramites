...
export default async function MedicamentosPage() {
  ...
  async function sellMedicineAction(formData: FormData) {
    ...
    redirect(`/medicamentos?error=${encodeURIComponent(result.message || 'No pudimos registrar el movimiento de stock.')}`);
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
        ...
        <h1 className="mt-3 text-3xl font-semibold text-white">Stock, recetas y vencimientos</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
          Controlá el stock disponible y también la vigencia de cada receta para trabajar con menos seguimiento manual.
        </p>
      </section>
                  ...
                  <div className="flex flex-col gap-2">
                    <form action={sellMedicineAction}>
                      ...
                      <input
                        ...
                        min="1"
                        defaultValue="1"
                        className="mb-2 w-24 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm outline-none"
                      />
                      <SubmitButton pendingText="Registrando..." className="rounded-2xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">
                        Registrar salida
                      </SubmitButton>
                    </form>
                    <form action={removeMedicineAction}>
                      <input type="hidden" name="medicineId" value={medicine.id} />
                      <SubmitButton pendingText="Borrando..." className="rounded-2xl border border-rose-400/20 bg-rose-400/10 px-4 py-2 text-sm font-medium text-rose-200">
                        Borrar
                      </SubmitButton>
                    </form>
                  </div>
                    ...
                    <div key={batch.id} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4 text-sm text-slate-300">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <form action={removeBatchAction}>
                          ...
                          <SubmitButton pendingText="Borrando..." className="rounded-full border border-rose-400/20 bg-rose-400/10 px-3 py-1 text-xs font-medium text-rose-200">
                            Borrar
                          </SubmitButton>
                        </form>
                      </div>
          ...
          <form action={addMedicineAction} className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              <label className="block">
                ...
                <input name="name" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
              </label>
              <label className="block">
                <span className="mb-2 block text-sm text-slate-300">Proveedor</span>
                <input name="supplier" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
              </label>
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <span className="mb-2 block text-sm text-slate-300">Precio compra</span>
                  <input type="number" name="purchasePrice" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
                </label>
                <label className="block">
                  <span className="mb-2 block text-sm text-slate-300">Precio venta</span>
                  <input type="number" name="salePrice" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
                </label>
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                <p className="text-sm font-medium text-white">Control de receta</p>
                <p className="mt-1 text-sm text-slate-400">
                  Opcional. Si lo completás, el sistema mostrará si la receta sigue vigente y por cuántos meses dura.
                </p>
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <DateField label="Fecha de receta" name="prescriptionIssuedAt" />
                  <label className="block">
                    <span className="mb-2 block text-sm text-slate-300">Duración en meses</span>
                    <input type="number" name="prescriptionDurationMonths" min="1" placeholder="Ej. 3" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
                  </label>
                </div>
              </div>
            </div>
            <SubmitButton pendingText="Guardando medicamento..." className="mt-5 w-full rounded-2xl bg-cyan-400 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-300">
              Guardar medicamento
            </SubmitButton>
          </form>
          ...
          <form action={addBatchAction} className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              <label className="block">
                ...
                <select name="medicineId" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400">
                  {medicines.map((medicine) => (
                    <option key={medicine.id} value={medicine.id} className="bg-slate-900">
                      {medicine.name}
                    </option>
                  ))}
                </select>
              ...
              <label className="block">
                ...
                <input name="batchNumber" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
              </label>
              <div className="grid gap-4 sm:grid-cols-2">
                <DateField label="Vencimiento" name="expirationDate" />
                <label className="block">
                  <span className="mb-2 block text-sm text-slate-300">Cantidad</span>
                  <input type="number" name="quantityAvailable" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-cyan-400" />
                </label>
              </div>
            </div>
            <SubmitButton pendingText="Guardando lote..." className="mt-5 w-full rounded-2xl bg-cyan-400 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-300">
              Guardar lote
            </SubmitButton>
          </form>
        </div>
