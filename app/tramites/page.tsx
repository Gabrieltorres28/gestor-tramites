    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_0.92fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-white">Seguimiento simple y claro</h1>
          <p className="mt-3 text-sm leading-6 text-slate-300">
            Creá cada trámite con el cliente correcto y avanzalo con acciones rápidas según el momento de la gestión.
          </p>
        </div>
        <NewProcedureForm
        ...
        {procedures.map((procedure) => (
          <article key={procedure.id} className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-lg shadow-slate-950/20">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-sm text-slate-400">{procedure.id}</p>
                <h2 className="mt-1 text-xl font-semibold text-white">{procedure.clientName}</h2>
                <p className="mt-2 text-sm text-slate-300">{procedure.description || 'Sin descripción.'}</p>
              </div>

              <div className="space-y-2 sm:w-56">
                {procedure.statusKey === 'IN_PROGRESS' ? (
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm text-slate-300 sm:grid-cols-5">
              <div>
                ...
                <p className="mt-1">{procedure.type}</p>
              </div>
              <div>
